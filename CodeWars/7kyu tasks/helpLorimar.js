...
Kata Link:
  https://www.codewars.com/kata/57102bbfd860a3369300089c/javascript
*/

function sensorAnalysis(sensor_data) {
  const { length } = sensor_data;
  let numbers = sensor_data
    .map((arr) => {
      return arr.filter((item) => !isNaN(item));
    })
    .reduce((acc, num) => acc.concat(num), []);
  const middleValue = +(
    numbers.reduce((sum, num) => sum + num, 0) / length
  ).toFixed(4);

  const squaresDeviations =
    numbers.reduce((sum, num) => {
      return sum + (num - middleValue) ** 2;
    }, 0) /
    (length - 1);
  const standDeviation = +Math.sqrt(squaresDeviations).toFixed(4);
  return [middleValue, standDeviation];
}

console.log(sensorAnalysis(sensor_data));
