...
function validateNumber(str) {
  ...
  if (numberStr.length === 11 || numberStr.length === 13) {
    if (numberStr.slice(0, 2) === "07") {
      if (numberStr.slice(2).length === 9) {
        return 'In with a chance';
      }
    }

    if (numberStr.slice(0, 4) === "+447") {
      if (numberStr.slice(4).length === 9) {
        return 'In with a chance';
      }
    }
  }
...
console.log(validateNumber("+4474-444444-44"));
